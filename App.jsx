...
// The Odds API sport keys for DraftKings
// ─── ODDS API CONFIG ─────────────────────────────────────────────────────────
// Sport keys confirmed from the-odds-api.com/liveapi/guides/v4/
// Note: use oddsFormat=american to get American odds directly — no conversion needed
// PGA tour uses "golf_pga_tour" key for regular tour events (not majors)
const API_SPORT_KEYS = [
  { key: "basketball_nba",   label: "NBA"   },
  { key: "baseball_mlb",     label: "MLB"   },
  { key: "basketball_ncaab", label: "NCAAB" },
  { key: "golf_pga_tour",    label: "PGA"   },
];
const REFRESH_MS       = 60 * 60 * 1000; // 60 minutes

// ─── ODDS API HELPERS ─────────────────────────────────────────────────────────
function parseOddsAPIGame(apiGame, sportLabel) {
  // Try DraftKings first, fall back to any US bookmaker so we always show something
  let bk = apiGame.bookmakers?.find(b => b.key === "draftkings");
  if (!bk && apiGame.bookmakers?.length) bk = apiGame.bookmakers[0];
  if (!bk) return null;
  ...
  const awayTeam = apiGame.away_team;

  // h2h (moneyline)
  const homeML    = h2h?.outcomes?.find(o => o.name === homeTeam);
  const awayML    = h2h?.outcomes?.find(o => o.name === awayTeam);
  // spread — home side
  const homeSprd  = spread?.outcomes?.find(o => o.name === homeTeam);
  const awaySprd  = spread?.outcomes?.find(o => o.name === awayTeam);
  // total — over/under
  const overLine  = totals?.outcomes?.find(o => o.name === "Over");

  const commenceTime = new Date(apiGame.commence_time);
  const now          = new Date();
  const diffMs       = commenceTime - now;
  const isLive       = diffMs < 0 && diffMs > -5 * 3600 * 1000;
  ...
  // API returns American odds directly when oddsFormat=american
  // Positive values are already +150 style; negative are -110 style
  const roundAmerican = (v) => {
    if (v == null) return null;
    // If decimal slipped through (>= 1.01 and < 100), convert
    if (v > 0 && v < 100) return Math.round((v - 1) * 100);
    if (v < 0 && v > -100) return Math.round(-100 / (v - 1));
    return Math.round(v);
  };
  ...
  return {
    ...
    away:        awayTeam,
    time:        et,
    homeOdds:    roundAmerican(homeML?.price),
    awayOdds:    roundAmerican(awayML?.price),
    spread:      homeSprd?.point ?? null,
    spreadOdds:  roundAmerican(homeSprd?.price) ?? -110,
    awaySpread:  awaySprd?.point ?? null,
    awaySpreadOdds: roundAmerican(awaySprd?.price) ?? -110,
    total:       overLine?.point ?? null,
    overOdds:    roundAmerican(overLine?.price) ?? -110,
    underOdds:   roundAmerican(totals?.outcomes?.find(o => o.name === "Under")?.price) ?? -110,
    status:      isLive ? "live" : diffMs < 0 ? "final" : "upcoming",
    bookmaker:   bk.key === "draftkings" ? "DK" : bk.title,
    lastUpdated: bk.last_update,
  };
}

async function fetchSportOdds(apiKey, sportKey, sportLabel) {
  // Call our Vercel serverless proxy (/api/odds) which forwards to the-odds-api.com
  // The proxy runs server-side so CORS restrictions don't apply.
  const url = `/api/odds?sport=${encodeURIComponent(sportKey)}&apiKey=${encodeURIComponent(apiKey)}`;
  ...
  if (!Array.isArray(data)) throw new Error("Unexpected response from proxy");
  const parsed = data.map(g => parseOddsAPIGame(g, sportLabel)).filter(Boolean);
  return { games: parsed, remaining: res.headers.get("x-requests-remaining") };
}

async function fetchAllOdds(apiKey) {
  const results = await Promise.allSettled(
    API_SPORT_KEYS.map(s => fetchSportOdds(apiKey, s.key, s.label))
  );
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (r.status === "fulfilled") {
      ...
      if (r.value.remaining) remaining = r.value.remaining;
    } else {
      errors.push(`${API_SPORT_KEYS[i].label}: ${r.reason?.message || "failed"}`);
    }
  }

  // Sort: live first, then upcoming by time, then finals
  games.sort((a, b) => {
    const order = { live: 0, upcoming: 1, final: 2 };
    const ao = order[a.status] ?? 1, bo = order[b.status] ?? 1;
    if (ao !== bo) return ao - bo;
    return a.time.localeCompare(b.time);
  });

  return { games, errors, remaining };
}

// Sharp data is still curated (no public betting % API available free)
const SHARP_DATA = [
  { id: 1, game: "Iowa vs Illinois – Elite 8",        bet: "Illinois -7.5",    betPct: 74, moneyPct: 81, sharpSide: "Illinois", movement: "-6.5 → -7.5",        steam: true,  reverseLineMove: false, sport: "NCAAB" },
  { id: 2, game: "Purdue vs Arizona – Elite 8",       bet: "Over 153.5",       betPct: 86, moneyPct: 96, sharpSide: "Over",     movement: "151.5 → 153.5",       steam: true,  reverseLineMove: false, sport: "NCAAB" },
  { id: 3, game: "Bulls @ Grizzlies",                 bet: "Bulls -4.5",       betPct: 58, moneyPct: 71, sharpSide: "Bulls",    movement: "-4 → -4.5",           steam: false, reverseLineMove: false, sport: "NBA"   },
  { id: 4, game: "Kings @ Hawks",                     bet: "Under 235.5",      betPct: 38, moneyPct: 67, sharpSide: "Under",    movement: "237 → 235.5",         steam: true,  reverseLineMove: false, sport: "NBA"   },
  { id: 5, game: "Rays @ Cardinals",                  bet: "Rays +ML",         betPct: 42, moneyPct: 64, sharpSide: "Rays",     movement: "Cardinals -118→-111", steam: false, reverseLineMove: true,  sport: "MLB"   },
  { id: 6, game: "Red Sox @ Reds",                    bet: "Over 7.5",         betPct: 44, moneyPct: 71, sharpSide: "Over",     movement: "7 → 7.5",             steam: true,  reverseLineMove: false, sport: "MLB"   },
  { id: 7, game: "Houston Open – PGA R3",             bet: "Min Woo Lee Top 5",betPct: 31, moneyPct: 68, sharpSide: "Lee",      movement: "+700 → +550",         steam: true,  reverseLineMove: false, sport: "PGA"   },
];
...
const API_KEY_STORAGE = "powerpickshq-oddsapi-key";
...
export default function App() {
  ...
  // ── Load picks + saved API key on mount ──
  useEffect(() => {
    // Load both in parallel
    Promise.all([loadPicks(), loadApiKey()]).then(([savedPicks, savedKey]) => {
      setPicks(savedPicks);
      if (savedKey) { setApiKey(savedKey); setApiKeyInput(savedKey); }
      setLoaded(true);
    }).catch(() => {
      setLoaded(true); // always unblock UI
    });
  }, []);
  ...
  const doFetch = async (key) => {
    ...
    setSportErrors([]);
    try {
      const { games: fresh, errors, remaining: rem } = await fetchAllOdds(key);
      setGames(fresh);
      setSportErrors(errors || []);
      if (rem) setRemaining(rem);
      setLastRefresh(new Date());
      setCountdown(REFRESH_MS / 1000);
      // If we got zero games AND have errors, surface the first error
      if (fresh.length === 0 && errors?.length) {
        const first = errors[0];
        if (first.includes("401") || first.includes("403")) {
          setOddsError("Invalid API key — double-check at the-odds-api.com");
        } else {
          setOddsError(`No games loaded. ${first}`);
        }
      }
    } catch (e) {
      const msg = e.message || "";
      setOddsError(
        msg.includes("401") || msg.includes("403")
          ? "Invalid API key — double-check at the-odds-api.com"
  ...
  useEffect(() => {
    if (!apiKey) return;
    doFetch(apiKey);
    const interval = setInterval(() => doFetch(apiKey), REFRESH_MS);
  ...
  const handleSaveApiKey = () => {
    ...
    if (!k) return;
    saveApiKey(k).catch(() => {}); // fire and forget
    setApiKey(k);
  };
      ...
      <main style={{ padding:"16px", maxWidth:1400, margin:"0 auto" }}>
        ...
        {tab === "odds" && (
          <div className="fade-up">
            ...
            {/* ── API Key setup / status bar ── */}
            {!apiKey ? (
              <div style={{ background:C.surface, border:`2px solid ${C.pitBlue}`, borderRadius:12,
                padding:"20px 24px", marginBottom:20, boxShadow:"0 4px 20px rgba(0,53,148,.1)" }}>
                <div style={{ fontSize:15, fontWeight:800, color:C.pitBlue, marginBottom:4 }}>
                  🔑 Connect Live DraftKings Odds
                </div>
                <div style={{ fontSize:12, color:C.muted, marginBottom:16, lineHeight:1.6 }}>
                  Enter your free API key from{" "}
                  <a href="https://the-odds-api.com" target="_blank" rel="noreferrer"
                    style={{ color:C.pitBlue, fontWeight:700 }}>the-odds-api.com</a>
                  {" "}to stream live DraftKings spreads, moneylines & totals — updated every 60 minutes.
                  Free tier includes 500 requests/month (no credit card needed).
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  <input
                    value={apiKeyInput}
                    ...
                    style={{ flex:1, padding:"10px 14px", borderRadius:8, fontSize:13,
                      border:`1px solid ${C.border}`, background:C.surfaceHi,
                      color:C.text, outline:"none", fontFamily:"'Roboto Mono',monospace" }}
                  />
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14,
                ...
                <span style={{ display:"flex", alignItems:"center", gap:5 }}>
                  {oddsLoading
                    ? <span style={{ width:8,height:8,borderRadius:"50%",border:`2px solid ${C.pitBlue}`,borderTopColor:"transparent",display:"inline-block",animation:"spin .6s linear infinite" }} />
                    : <span style={{ width:8,height:8,borderRadius:"50%",background:"#1A7A3F",boxShadow:"0 0 6px #1A7A3F",animation:"pulse 2s infinite" }} />}
                  <span style={{ fontSize:11, fontWeight:700, color:oddsLoading?C.muted:C.win }}>
                    {oddsLoading ? "Refreshing…" : "LIVE · DraftKings"}
                  </span>
                </span>
                {oddsError && <span style={{ fontSize:11, color:C.loss, fontWeight:600 }}>⚠ {oddsError}</span>}
                {lastRefresh && (
                  <span style={{ fontSize:10, color:C.muted }}>
                    Updated {lastRefresh.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})} · Next in <strong style={{ color:C.pitBlue }}>{mins}:{secs}</strong>
                  </span>
                )}
                <span style={{ fontSize:10, color:C.muted }}>
                  {games.length} game{games.length !== 1 ? "s" : ""}
                  {remaining !== null && ` · ${remaining} API credits left`}
                </span>
                {sportErrors.length > 0 && (
                  <span style={{ fontSize:10, color:"#D4500A" }} title={sportErrors.join(" | ")}>
                    ⚠ {sportErrors.length} sport{sportErrors.length > 1 ? "s" : ""} unavailable
                  </span>
                )}
                <button onClick={()=>doFetch(apiKey)} disabled={oddsLoading}
                  style={{ marginLeft:"auto", padding:"4px 12px", borderRadius:6,
                    border:`1px solid ${C.border}`, background:"transparent",
                    color:C.pitBlue, cursor:"pointer", fontSize:11, fontWeight:700,
                    fontFamily:"'Montserrat',sans-serif", opacity:oddsLoading?.5:1 }}>
                  ↻ Refresh now
                </button>
                <button onClick={()=>{
                  clearApiKey().catch(() => {});
                  setApiKey(""); setApiKeyInput(""); setGames([]);
            ...
            {oddsError && apiKey && (
              <div style={{ padding:"12px 16px", background:"#FFF0F2", border:`1px solid ${C.loss}55`,
                borderRadius:8, marginBottom:14, fontSize:12, color:C.loss, fontWeight:600 }}>
                ⚠ {oddsError}
              </div>
            )}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, flexWrap:"wrap", gap:10 }}>
              <div>
                ...
                <div style={{ fontSize:11, color:C.muted, marginTop:2 }}>
                  {apiKey ? "Live DraftKings lines · auto-refreshes every 60 min" : "Connect your API key above to load live lines"}
                </div>
              </div>
            ...
            {/* Loading skeleton */}
            {oddsLoading && games.length === 0 && (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))", gap:12 }}>
                {[1,2,3,4,5,6].map(i=>(
                  <div key={i} style={{ background:C.surface, borderRadius:10, padding:16,
                    border:`1px solid ${C.border}`, height:180,
                    background:`linear-gradient(90deg,${C.bgAlt} 25%,${C.surface} 50%,${C.bgAlt} 75%)`,
            ...
            {!apiKey && (
              <div style={{ textAlign:"center", padding:"48px 20px", color:C.muted }}>
                ...
                <div style={{ fontSize:15, fontWeight:700, color:C.pitBlue, marginBottom:6 }}>No live data yet</div>
                <div style={{ fontSize:13 }}>Enter your free Odds API key above to stream live DraftKings lines.</div>
              </div>
            )}

            {apiKey && !oddsLoading && games.length === 0 && !oddsError && (
              <div style={{ textAlign:"center", padding:"48px 20px", color:C.muted }}>
                <div style={{ fontSize:36, marginBottom:12 }}>🏆</div>
                <div style={{ fontSize:13 }}>No games currently available for the selected sport.</div>
              </div>
            )}
